...
export default class ApplyPromocodeBlock extends BaseComponent {
     ...
     * @param {boolean} result результат действия с промокодом,
     * false - отрицательный, true - положительный
     * @param {String} message сообщение, поясняющее результат
     */
    showPromocodeTryResult(result, message) {
        const promocodeStatusText = document.getElementById('cart-promocode-status');
        if (promocodeStatusText) {
     ...
     * @return {Object} контекст отрисовки
     */
    prepareRenderData() {
        return {
